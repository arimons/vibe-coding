...
> **이번 파트 목표:** 업체마다 양식이 다른 임상 보고서 PDF에서 원하는 값을 찾아내고, AI와 함께 추출 코드를 만들어 Excel에 자동 정리한다.
...
스캔본 페이지에서 값이 필요한 경우 가장 먼저 확인할 것은, 스캔본이 PDF에 **embedded 이미지로 첨부**되어 있는지입니다. 그렇다면 pymupdf로 그 이미지를 바로 추출해서 Gemini에 보내면 됩니다. 페이지 전체를 렌더링하거나 좌표를 계산할 필요가 없습니다.

> **AI 프롬프트 예시 (embedded 이미지 추출)**
>
> "pymupdf로 PDF 특정 페이지에 내장된 이미지를 추출하는 코드를 만들어줘.
> 추출된 이미지를 Gemini에 보내서 'P숫자' 형태의 값만 뽑아줘."

embedded 이미지 추출로 해결이 안 되는 경우(페이지 일부만 필요하거나, 이미지가 내장이 아닌 경우)에는 좌표 기반으로 영역을 잘라냅니다. 이때 좌표를 잡는 방법은 세 가지입니다.

**방법 1 — 렌더링 이미지에서 직접 읽기**
`debug_render.py`로 해당 페이지를 PNG로 저장한 뒤, 이미지 뷰어(macOS 미리보기, Windows 그림판)에서 마우스를 올리면 픽셀 좌표가 표시됩니다. 200 DPI 렌더링 기준으로 픽셀값을 200으로 나누고 72를 곱하면 PDF 포인트로 변환됩니다.

**방법 2 — 비율로 지정하기**
"가로 3~45%, 세로 48~56% 위치"처럼 페이지 비율로 설명하면 AI가 포인트값으로 계산해줍니다. 스캔 기기가 달라도 비율은 유지되어 더 안정적입니다.

**방법 3 — 주변 텍스트를 앵커로 잡기**
스캔본에도 텍스트 레이어가 일부 있다면, "표준시료 MEDs라는 텍스트 바로 아래 영역"처럼 상대 좌표로 지정할 수 있습니다. 절대 좌표보다 훨씬 안정적입니다.

> 처음에는 방법 1로 빠르게 잡고, 피험자마다 위치가 달라지면 방법 2나 3으로 전환합니다.
...
새 파일을 만드는 것과 **이미 있는 양식(템플릿)에 채워넣는 것**은 접근 방식이 다릅니다. CRF처럼 정해진 양식이 있는 경우, 빈 템플릿 파일을 복사해서 열고 정해진 셀 위치에 값을 넣는 방식을 사용합니다. 템플릿의 서식, 수식, 레이아웃을 건드리지 않고 값만 채워넣기 때문입니다.
...
템플릿의 어느 셀에 어떤 값이 들어가는지는 Excel 파일을 직접 열어 셀 주소(B3, D7 등)를 확인한 뒤 AI에게 알려주면 됩니다.
...
- [ ] AI에게 반복 구조 설명 → 순서대로 읽는 코드 생성
- [ ] 덤프 결과 붙여넣기로 텍스트 추출 코드 생성
- [ ] 스캔본 embedded 이미지 추출 시도 → 안 되면 좌표 방법 선택
- [ ] 부분 이미지 → AI OCR로 값 추출 성공
- [ ] 전체 피험자 자동 처리 → Excel 템플릿 기입 완료
