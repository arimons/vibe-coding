...
**나만의 PDF → Markdown 변환기, 처음부터 직접 만들기**

강의에서 배운 내용을 바탕으로 스크립트를 직접 설계하고 만들어봅니다.
기존에 제공된 스크립트는 참고하지 않고, AI와 단계별로 대화하면서 처음부터 만드는 것이 핵심입니다.

---

### 대상 문서 준비

[SCCS Opinions 목록](https://health.ec.europa.eu/scientific-committees/scientific-committee-consumer-safety-sccs/sccs-opinions_en)에서 관심 있는 성분 의견서 PDF 1개를 다운로드합니다.

> 예시: Retinol, Hyaluronic acid, Niacinamide 등 익숙한 성분으로 찾으면 읽는 재미도 있습니다.  
> SCCS는 EU 공공기관 문서라 저작권 걱정 없이 사용할 수 있습니다.

---

### 수행 방법

아래 흐름을 **직접 생각하고** 각 단계를 AI에게 따로따로 요청해서 코드를 만들어봅니다.  
한 번에 전부 만들어달라고 하지 말고, 단계를 쪼개서 하나씩 완성하는 것이 중요합니다.

```
1단계: PDF 파일을 열어서 총 페이지 수 확인
        ↓
2단계: 페이지를 이미지(PNG)로 변환
        ↓
3단계: 이미지를 Gemini에 전달해서 텍스트를 Markdown 형식으로 추출
        ↓
4단계: 전체 페이지 결과를 하나의 output.md 파일로 저장
```
...
```text
pymupdf(fitz)로 PDF 파일을 열어서 총 페이지 수를 출력하는 코드를 만들어줘.
PDF 경로는 변수로 받게 해줘.
```
...
```text
방금 만든 코드에 기능을 추가해줘.
각 페이지를 PNG 이미지로 변환해서 output_images 폴더에 저장하게 해줘.
파일명은 page_001.png, page_002.png 형식으로 저장해줘.
```

**동작 확인 후 3단계:**

```text
page_001.png 이미지 한 장을 Gemini에게 보내서
페이지 내용을 Markdown 형식으로 변환해달라고 요청하는 코드를 추가해줘.
API 키는 .env 파일에서 읽어오게 해줘. (google-genai 패키지 사용)
```

**동작 확인 후 4단계:**

```text
1장이 잘 되면, 전체 페이지를 순서대로 처리해서
결과를 하나의 output.md 파일로 저장하게 바꿔줘.
```
...
| 단계 | 패키지 | 비고 |
|------|--------|------|
| PDF → 이미지 | `pymupdf` (`import fitz`) | Week 5 실습과 동일 |
| Gemini 호출 | `google-genai` | Week 5 실습과 동일 |
| 환경변수 | `python-dotenv` | `.env`에서 API 키 로드 |

신규 설치 없이 Week 5 환경 그대로 사용합니다.
...
- 만든 Python 스크립트 파일
- 생성된 `output.md` 파일 (첫 3페이지만 처리해도 OK)
- 짧은 메모: 어떤 프롬프트로 요청했는지, 어디서 막혔는지, AI와 어떻게 해결했는지

> 완벽하게 동작하지 않아도 괜찮습니다.  
> 막힌 지점을 AI에게 어떻게 다시 물어봤는지가 더 중요한 학습입니다.
