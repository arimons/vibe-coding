...
지난 5주차의 실습과 과제에서는 문서 통째로의 텍스트를 마크다운(글) 형태로 쭉 뽑아보았습니다.
하지만 진짜 실무에서는 "단순히 글을 빼내는 것"보다, 빼낸 데이터를 **엑셀이나 사내 데이터베이스(DB) 시스템에 넣기 위해 줄을 맞추는 일**이 훨씬 중요합니다.

LLM은 기본적으로 특별한 지시가 없으면 **자유 텍스트**로 줄글을 길게 답합니다.

**일반적인 요청 (단순 프롬프트)**
```
질문: 이 원료 COA(성적서)에서 주요 수치를 알려줘.
답변: 네, 이 문서에 따르면 원료명은 비공개(Redacted)이며, 순도는 98.5% 이상입니다. 또한 수분 함량은 0.5% 이하이며, 보관 조건은...
```
이렇게 줄글로 답변이 오면, 실무자는 결국 이걸 또 사람이 눈으로 읽고 엑셀에 `순도 98.5%`, `수분 0.5%`를 일일이 다시 칸에 맞춰서 타이핑해야 합니다.

**Structured Output 요청 (구조화 출력)**
우리가 원하는 것은 LLM에게 **"불필요한 인사말 빼고, 딱 내가 정해준 엑셀 단어(표나 JSON)에만 데이터를 채워넣어!"**라고 강제하는 것입니다.
...
이렇게 출력되면, 우리는 그 결과물을 복사해서 엑셀에 한 번에 붙여넣거나, 시스템에 무인으로 바로 전송시킬 수 있습니다. 즉 데이터 후가공 노가다를 없애줍니다.
...
## ⚠️ 실습용 데이터 준비 및 보안 (COA, MSDS)

5주차 5-B 실습(임상 보고서 로컬 처리 파이프라인)에서 우리는 **"민감한 사내 데이터는 외부 AI 서비스(Public LLM)에 통째로 전송하면 안 된다"**는 원칙을 배웠습니다.

따라서 이번 구조화 출력 실습에서도 진짜 보안 문제가 일절 없는 실무형 문서들을 활용합니다.

1. **MSDS (물질안전보건자료)**: 제품의 안전성 정보를 담은 파일로, 사실상 외부에 널리 전체 공개되는 자료이므로 보안 리스크가 전혀 없습니다.
2. **원료 COA (시험성적서)**: 엑셀이나 내부 시스템 ERP에 가장 빈번하게 적어 넣어야 하는 서류의 꽃입니다. 특정 회사의 대외비나 핵심 원료명이 들어간다면, **원료명 등 핵심 정보 부분만 화이트로 지운(Redacted) 실제 COA 파일**을 쓰시면 보안 걱정 없이 생생한 실무 느낌으로 실습이 가능합니다.
...
<div style="display:flex; gap:16px; margin:20px 0;">
  ...
  <div style="flex:1; border:1.5px solid #d0d0d0; border-radius:8px; padding:16px; background:#fafafa;">
    <div style="font-weight:bold; color:#888; margin-bottom:10px;">❌ 일반 프롬프트</div>
...
Week 7에서는 인터넷상에 멀리 떨어져 있는 수많은 문서 데이터를 파도를 타며 가져오는 **웹 스크래핑**을 다룹니다.
데이터 분석가들이 쓰는 복잡한 파이썬 스크래핑 라이브러리(BeautifulSoup, Selenium 등) 기술 구조를 전혀 아실 필요 없습니다.
웹사이트 주소(URL) 하나만 AI 채팅방에 툭 던져주면, AI가 알아서 사이트에 접속하는 스크래핑 코드를 짜고 여러분이 요구한 JSON 양식/엑셀 양식으로 가지런히 싹 쓸어담아 줍니다!
